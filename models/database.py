...
# 之后这个钩子没有了，而且本项目所有路由都是 async def（跑在同一个事件循环线程上），
# 于是 thread-local 作用域会退化成：**整个进程共用同一个 Session，永不复位**。
#
# 后果不是"慢"，而是"错"：
...
# 令牌放在 ContextVar 里 —— 它能被复制进 anyio 的工作线程，所以即便以后路由改成同步
# 函数（走线程池）也依然能正确清理。请求之外（定时任务、日志消费线程等）回退到线程 ID，
# 与原有行为一致。
